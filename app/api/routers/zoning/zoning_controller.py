...
def generate_zones(profile : zoning_models.Profile, blocks : zoning_models.BlocksModel, max_iter : int = 1_000) -> zoning_models.ZonesModel:
    blocks_gdf = gpd.GeoDataFrame.from_features([f.model_dump() for f in blocks.features], const.DEFAULT_CRS)
    return zoning_service.generate_zoning(profile, blocks_gdf, max_iter)